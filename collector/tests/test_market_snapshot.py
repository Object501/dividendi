...
from collector.dividendi_data.market_snapshot import parse_market_snapshot
...
class MarketSnapshotTest(unittest.TestCase):
    ...
    def test_loads_valid_document(self) -> None:
        document = parse_market_snapshot(self.raw_document, self.catalog)
    ...
    def test_rejects_inconsistent_discount(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "discountPoints 与原始行情不一致"):
            parse_market_snapshot(document, self.catalog)
