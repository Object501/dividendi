...
def main() -> None:
    ...
    validate = subparsers.add_parser("validate-data", help="校验待发布的历史数据")
    validate.add_argument("--history", type=Path, default=DEFAULT_HISTORY_PATH)
    arguments = parser.parse_args()

    if arguments.command == "update-history":
        changed = refresh_history(arguments.output)
        print("日终历史已更新" if changed else "日终历史未变化。无需更新")
    elif arguments.command == "backfill-history":
        result = backfill_history(arguments.output)
