...
自动补齐，并删除 PR 标题中已有的 `(#N)` 后缀。校验时会临时加上当前 PR 号，以确保
GitHub 生成的最终 squash 提交仍符合格式且不超过 72 个字符。squash merge 的正文
留空，因此 Dependabot PR 应使用 squash；只有分支中每条提交本身都符合格式时才使用 rebase。
